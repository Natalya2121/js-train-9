...
function findElementGreaterThan(arr, num) {
  ...
  if (Array.isArray(arr)) {
    let res = arr.find((value) => value > num);
    if (res == undefined) return "Числа яке задовільняе умову не знайдено";
    else return res;
...
function findIndexByProperty(arr, name) {
  ...
  if (Array.isArray(arr)) return arr.findIndex((value) => value.name === name);
  else return -1;
}
...
function findLastEvenNumber(arr) {
  ...
  if (Array.isArray(arr)) {
    ...
    if (res == undefined) return "Парних чисел не знайдено";
    else return res;
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  // Повертаємо рядок.
  if (Array.isArray(arr)) return arr.slice(startIdx).join(" ");
  else return "Вхідний параметр має бути масивом";
}
...
function getArrayKeys(arr) {
  ...
  if (Array.isArray(arr)) return Array.from(arr.keys());
  else return "Вхідний параметр має бути масивом";
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (Array.isArray(arr)) {
    ...
    return arr1.reduceRight((sum, value) => sum + value, 0);
  } else return "Вхідний параметр має бути масивом";
}
...
function expandArrayByN(arr, n) {
  ...
  if (Array.isArray(arr) && typeof n == "number") {
    return arr.flatMap((value) => Array(n).fill(value));
...
function findLongestWord(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.reduce(
      (word, value) => (value.length > word.length ? value : word),
      ""
    );
  } else return "";
}
...
function findDuplicateElements(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.filter((value, index, ar) => index !== ar.indexOf(value));
  } else return [];
}
...
function calculateTotalPrice(arr) {
  ...
  if (Array.isArray(arr)) {
    return arr.reduce((sum, value) => sum + value.price, 0);
  } else return 0;
}
